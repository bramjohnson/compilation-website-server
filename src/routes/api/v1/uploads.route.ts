...
export function setupUploadsRouter(prismaClient: PrismaClient): Router {
    ...
    router.post(
        ...
        upload.single("image"),
        async (req: RequestWithResolvableUser, res: Response, next: NextFunction) => {
            ...
            try {
                ...
                if (!Object.values(ImageType).includes(type)) {
                    res.status(400).json({ error: `Invalid image type. Must be one of: ${Object.values(ImageType).join(", ")}` });
                    return;
                }
    ...
    router.get("/:id", async (req: Request, res: Response, next: NextFunction) => {
        try {
            const image = await prismaClient.image.findUnique({ where: { id: req.params.id } });
        ...
        async (req: Request, res: Response, next: NextFunction) => {
            try {
                ...
                if (!w || !h || w < 1 || h < 1 || w > 4096 || h > 4096) {
                    res.status(400).json({ error: "Query params w and h are required and must be between 1–4096." });
                    return;
                }
    ...
    router.delete("/:id", async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            // Delete original
            await fs.unlink(image.path).catch(() => {/* already gone, no-op */ });

            // Delete any cached variants matching this image id
            const cacheFiles = await fs.readdir(CACHE_DIR).catch(() => [] as string[]);
            await Promise.all(
                cacheFiles
