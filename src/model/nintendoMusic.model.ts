...
export async function createNintendoMusicTrack(
  prismaClient: PrismaClient,
  title: string,
  durationMillis: number,
  nintendoMusicAlbumId: number,
  nintendoMusicId: string,
  ...
  if (updatedUserDefinedTracks.count > 0) {
    ...
    async function findOrCreateUserDefinedAlbum(
      gameName: string,
    ): Promise<UserDefinedAlbum> {
      const existingUserDefinedAlbum =
        await prismaClient.userDefinedAlbum.findUnique({
          where: {
            name: gameName,
          },
        });
    ...
    const userDefinedAlbum = await findOrCreateUserDefinedAlbum(
      nintendoMusicTrack.nintendoMusicLibraryGameAlbum.name,
    );
