...
function getRawNintendoMusicResponses(): [
  string[],
  RawOfficialPlaylistsResponse[],
] {
  const rawResponseFilePaths = readdirSync("./mitmed/raw/officialPlaylists");
  const rawResponseBodyFilePaths = rawResponseFilePaths.filter((filePath) =>
    filePath.endsWith(".body.json"),
  );
  const rawRespones = rawResponseBodyFilePaths.map((filePath) => {
  ...
  return [rawResponseBodyFilePaths, rawJsonResponses];
}
...
async function applyExtractedData(
  ...
  async function resolveGameAlbum(): Promise<NintendoMusicLibraryGameAlbum> {
    ...
    if (existingGameAlbum !== null) {
      console.debug(
        `Resolved ${data.game.id} ${data.game.name} to existing identical game album.`,
      );
    ...
    const existingSimilarGameAlbum =
    ...
    if (existingSimilarGameAlbum !== null) {
      console.debug(
        `Resolved ${data.game.id} to existing similarly named game album ${existingSimilarGameAlbum.id} ${existingSimilarGameAlbum.name}`,
      );
      const updatedSimilarGameAlbum =
        await prismaClient.nintendoMusicLibraryGameAlbum.update({
          data: gameData,
          where: {
    ...
    console.debug(
      `Resolved ${data.game.id} ${data.game.name} by creating a new game album`,
    );
  ...
  async function resolveGameTrack(
    parentGame: NintendoMusicLibraryGameAlbum,
    track: ExtractedNintendoMusicTrack,
  ): Promise<NintendoMusicLibraryTrack> {
    const trackData = {
      title: track.name,
    ...
    const existingTrack =
      await prismaClient.nintendoMusicLibraryTrack.findUnique({
        where: {
          nintendoMusicId: track.id,
        },
      });
    ...
    if (existingTrack !== null) {
      console.debug(
        `Resolved ${track.id} ${track.name} to existing identical track.`,
      );
    ...
    const existingSimilarTrack =
      await prismaClient.nintendoMusicLibraryTrack.findFirst({
        where: {
    ...
    if (existingSimilarTrack !== null) {
      console.debug(
        `Resolved ${track.id} to existing similarly named track ${existingSimilarTrack.id} ${existingSimilarTrack.title}`,
      );
      return await prismaClient.nintendoMusicLibraryTrack.update({
        data: trackData,
        where: {
    ...
    return await createNintendoMusicTrack(
      prismaClient,
      track.name,
  ...
  console.info(
    `Resolved ${resolvedTracks.length} tracks from ${data.game.name}!`,
  );
}

function main() {
  const [rawPaths, rawResponses] = getRawNintendoMusicResponses();
  const extracteds = rawResponses.map((response) =>
    extractedNintendoMusicFromRawNintendoMusic(response),
  );

  const prismaClient = createPrismaClient();
  extracteds.forEach((extracted) =>
    applyExtractedData(prismaClient, extracted),
  );
